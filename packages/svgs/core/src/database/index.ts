...
type IconsTable<T extends IconMetaData> = Map<string, T>; // json 파일에 저장된/저장할 테이블 (아이콘 이름 -> 아이콘 메타데이터)
 ...
 */
export class IconsDatabaseBaseManager<T extends IconMetaData> {
  private readonly FILE_PATH: string;

  private iconsTable: IconsTable<T>;
